...
export function undoMove(cube: RubikCube): void {
    ...
    rotateFace(cube, face, direction === RotationDirection.Clockwise ? RotationDirection.CounterClockwise : RotationDirection.Clockwise);
}
...
function generateScramble(length: number): Move[] {
    const moves = Object.values(Move).filter((v) => typeof v === 'string') as Move[];
    const scramble = [];
...
export function scrambleCube(cube: RubikCube, scrambleLength: number = 20): void {
